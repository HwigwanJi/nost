...
| ID | 이름 | 시간 | 핵심 동작 |
|---|---|---|---|
| `cards.scan` | 스마트 스캔으로 빠르게 추가 | ~90s | 사이드바 💡 클릭 → RecommendPanel 열림 → 추천 1개 클릭 |
| `cards.clipboard` | 클립보드 복사로 빠르게 추가 | ~120s | URL 복사 → 게이트웨이 배너 등장 → URL 카드 변환 |
| `cards.memo` | 메모도 카드처럼 | ~120s | 텍스트 복사 → 게이트웨이 → "메모로" → 마크다운 자동 정리 시연 |
| `cards.dragdrop` | 드래그 앤 드롭으로 추가 | ~60s | 파일 드래그 → 스페이스에 드롭 (실제 파일 안 만들고 가상 드래그 이벤트) |
| `cards.dialog` | 다이얼로그 깊이 알기 | ~150s | + 추가 → 3-phase 다이얼로그 한 사이클 → 화면 픽 모드 체험 |

### 3.3 카테고리 3. 자유롭게 배치하기 (Layout)

| ID | 이름 | 시간 | 핵심 동작 |
|---|---|---|---|
| `layout.cardmove` | 카드 이동하기 | ~60s | 카드 우클릭 드래그 → 다른 위치로. 우클릭임을 명시 (좌클릭은 swipe-gesture로 충돌) |
| `layout.spacereorder` | 스페이스 순서 바꾸기 | ~60s | 스페이스 그립 좌클릭 드래그 → 위/아래 |
| `layout.tile` | 타일링 알아보기 | ~90s | 노드 그룹 빌드 → 타일링 실행 (모니터 1개로도 가능한 시나리오) |

### 3.4 카테고리 4. 심화 기능 (Advanced)

| ID | 이름 | 시간 | 핵심 동작 |
|---|---|---|---|
| `advanced.floating` | 플로팅 뱃지 만들기 | ~120s | 카드 우클릭 → 플로팅으로 → 화면 어디든 떠있는 뱃지 확인 |
| `advanced.nodegroup` | 노드 그룹 (한 번에 여러 개 실행) | ~150s | 노드 모드 진입 → 카드 2~3개 묶기 → 단축키로 한 번에 실행 |
| `advanced.preset` | 프리셋 전환 | ~90s | 프리셋 1↔2 토글 (Pro 사용자만 — 무료는 락 화면 + "이 퀘스트는 유료" 안내) |

### 3.5 카테고리 5. 위젯 알아보기 (Widgets)

| ID | 이름 | 시간 | 핵심 동작 |
|---|---|---|---|
| `widgets.music` | 음악 위젯 | ~60s | 음악 위젯 추가 → 재생/일시정지 swipe → 볼륨 슬라이더 |
| `widgets.color` | 컬러 코드 위젯 | ~90s | 헥스 복사 → 게이트웨이로 위젯 추가 → swipe (보색/유사색 복사) |
| `widgets.memo` | 메모 위젯 | ~90s | 빈 메모 추가 → 에디터 열기 → 정리 도구 팔레트 한 번 사용 |
...
## 11. 미해결 질문 (구현 전 확정 필요)

1. **보상 일수 단위** — 1일 단위가 적당한지 (혹은 "캐시 N원" 류) → 사용자 결정
2. **첫 가입 vs 재방문 사용자 진입점** — 신규는 자동으로 카테고리 1 nudge? 재방문은 명시적만? → 사용자 결정
3. **퀘스트 일시중단 후 재개 UI** — 설정 아코디언 외에 토스트로도 "이어서 하기" 노출할지
4. **Nudge 기본값 (사용자가 한 번도 dismiss 안 했을 때)** — show vs ask first? 현재 spec은 show
