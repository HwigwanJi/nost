...
export function TourOverlay({ onComplete }: Props) {
  ...
  useEffect(() => {
    const onStart = (e: Event) => {
      ...
      if (!id) {
        // No id given — pick the first uncompleted, or just the first.
        const first = TOURS[0];
        if (first) { setTour(first); setStepIdx(0); }
        return;
      }
      const t = findTour(id);
      if (t) { setTour(t); setStepIdx(0); }
    };
    window.addEventListener('nost:start-tour', onStart);
    return () => window.removeEventListener('nost:start-tour', onStart);
  }, []);
      ...
      <div style={popoverStyle} role="dialog" aria-modal="true">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {step.advanceOn !== 'target-click' && step.advanceOn !== 'condition' && (
              <button
                onClick={advance}
                style={{
